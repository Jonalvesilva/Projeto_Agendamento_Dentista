...
import { IWeekDataDialog } from "@/app/_interfaces/interfaces";

import { FaRegEdit } from "react-icons/fa";
import { MdDelete } from "react-icons/md";
import { formatarHora } from "@/app/_functions/hour";

export default function DialogHourItem({ data, day, hour }: IWeekDataDialog) {
  const slotHour = Number(`${hour[0]}${hour[1]}`);

  const initialHour = `${slotHour.toLocaleString().padStart(2, "0")}:00`;
  const lastHour = `${(slotHour + 1).toLocaleString().padStart(2, "0")}:00`;
    ...
    <DialogContent className="w-[90vw] max-w-screen-md rounded-md ">
      <DialogHeader>
        <DialogTitle className="text-start">
          ...
          {String(initialHour)} e {String(lastHour)}
        </DialogTitle>
      </DialogHeader>
      ...
      <div>
        {arr.length > 0 ? (
          ...
          <div className="flex justify-between items-center">
            <h2 className="text-[16px]">Sem eventos no periodo selecionado</h2>
